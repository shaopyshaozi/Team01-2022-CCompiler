#ifndef AST_BINARY_GREATERTHANEQUAL_HPP
#define AST_BINARY_GREATERTHANEQUAL_HPP

#include "ast_header/ast_Base.hpp"

class BinaryGreaterThanEqual : public Base {
public:
    BinaryGreaterThanEqual(BasePtr left, BasePtr right);
    virtual ~BinaryGreaterThanEqual();

    void Print(std::ostream &stream, std::string indent) const override;

    void generateCode(std::ostream& stream, Context &context, int destReg) const override;

    int getSize() const override;
};

#endif /* AST_BINARY_ADD_HPP */

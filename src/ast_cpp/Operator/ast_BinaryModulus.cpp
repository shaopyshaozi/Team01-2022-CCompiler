...
BinaryModulus::BinaryModulus(BasePtr left, BasePtr right)
{ branches.insert(branches.end(), left);
  branches.insert(branches.end(), right);
}

BinaryModulus::~BinaryModulus()
{
  delete branches[0];
  delete branches[1];
    // No Modulusal cleanup needed
}

void BinaryModulus::Print(std::ostream &stream, std::string indent) const
{
...
void BinaryModulus::generateCode(std::ostream& stream, Context &context, int destReg) const
{
...
//-------------------------------------------------------------------------------------------------------------
    ...
    stream << "rem " << context.int_to_register(destReg) << "," << left_reg << "," << right_reg << std::endl;
...
int BinaryModulus::getSize() const{

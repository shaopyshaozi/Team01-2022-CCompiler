...
BinaryOR::BinaryOR(BasePtr left, BasePtr right)
...
BinaryOR::~BinaryOR()
{
  delete branches[0];
  delete branches[1];
    // No ORal cleanup needed
}

void BinaryOR::Print(std::ostream &stream, std::string indent) const
{
...
void BinaryOR::generateCode(std::ostream& stream, Context &context, int destReg) const
{
...
//-------------------------------------------------------------------------------------------------------------
    ...
    stream << "or " << context.int_to_register(destReg) << "," << left_reg << "," << right_reg << std::endl;

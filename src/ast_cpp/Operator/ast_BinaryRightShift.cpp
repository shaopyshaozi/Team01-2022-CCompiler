...
BinaryRightShift::BinaryRightShift(BasePtr left, BasePtr right)
{ branches.insert(branches.end(), left);
  branches.insert(branches.end(), right);
}

BinaryRightShift::~BinaryRightShift()
{
  delete branches[0];
  delete branches[1];
...
void BinaryRightShift::Print(std::ostream &stream, std::string indent) const
{
...
void BinaryRightShift::generateCode(std::ostream& stream, Context &context, int destReg) const
{
...
//-------------------------------------------------------------------------------------------------------------
    ...
    stream << "sra " << context.int_to_register(destReg) << "," << left_reg << "," << right_reg << std::endl;
...
int BinaryRightShift::getSize() const{

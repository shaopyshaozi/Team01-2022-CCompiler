...
BinaryNotEqual::BinaryNotEqual(BasePtr left, BasePtr right)
{ branches.insert(branches.end(), left);
  branches.insert(branches.end(), right);
}

BinaryNotEqual::~BinaryNotEqual()
{
  delete branches[0];
  delete branches[1];
...
void BinaryNotEqual::Print(std::ostream &stream, std::string indent) const
{
...
void BinaryNotEqual::generateCode(std::ostream& stream, Context &context, int destReg) const
{
...
//-------------------------------------------------------------------------------------------------------------
    ...
    stream << "snez " << right_reg << "," << right_reg << std::endl;
...
int BinaryNotEqual::getSize() const{

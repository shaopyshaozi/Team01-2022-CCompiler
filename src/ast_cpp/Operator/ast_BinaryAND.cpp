...
BinaryAND::BinaryAND(BasePtr left, BasePtr right)
...
void BinaryAND::Print(std::ostream &stream, std::string indent) const
{
...
void BinaryAND::generateCode(std::ostream& stream, Context &context, int destReg) const
{
...
//-------------------------------------------------------------------------------------------------------------
    ...
    stream << "and " << context.int_to_register(destReg) << "," << left_reg << "," << right_reg << std::endl;
